...
func (m Metrics) InstrumentedHandler(handler string) func(http.Handler) http.Handler {
	counter := m.counter.MustCurryWith(prometheus.Labels{"handler": handler})
	duration := m.duration.MustCurryWith(prometheus.Labels{"handler": handler})
	return func(next http.Handler) http.Handler {
		return promhttp.InstrumentHandlerCounter(counter, promhttp.InstrumentHandlerDuration(duration, next))
	}
}
