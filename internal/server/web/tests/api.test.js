...
global.fetch = vi.fn(async (url, options) => {
    ...
    if (url.startsWith('/api/v1/sessions/session/')) {
...
test('deleteSession calls correct URL', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith('/api/v1/sessions/session/id1', expect.any(Object));
});
